...
def resize_unscale(img, new_shape=(640, 640), color=114):
    ...
    pad_w, pad_h = new_shape[1] - \
        new_unpad_w, new_shape[0] - new_unpad_h  # wh padding
...
def infer_yolop(weight="yolop-640-640.onnx",
    ...
    for i in range(boxes.shape[0]):
        x1, y1, x2, y2, conf, label = boxes[i]
        x1, y1, x2, y2, label = int(x1), int(y1), int(x2), int(y2), int(label)
        img_merge = cv2.rectangle(
            img_merge, (x1, y1), (x2, y2), (0, 255, 0), 2, 2)
    ...
    return img_merge, da_seg_mask, ll_seg_mask


def laneLineDetect(weight, img_path):
    img_merge, da_seg_mask, ll_seg_mask = infer_yolop(
        weight=weight, img_path=img_path)

    ll_seg_mask_resized = cv2.resize(
        ll_seg_mask, (640, 480), interpolation=cv2.INTER_LINEAR)

    cv2.imshow('ll_seg_mask_resized', ll_seg_mask_resized)
    cv2.waitKey()
...
if __name__ == "__main__":
    ...
    #                     default="./inference/images/9aa94005-ff1d4c9a.jpg")
    parser.add_argument('--img', type=str,
                        default="mirv_lane_lines.png")
    args = parser.parse_args()

    print(laneLineDetect(weight=args.weight, img_path=args.img))
    """
